...
//        $this->assign("menu_id",Detection_menu());
        //行政区划信息
        $commodity_model = M("commodity");
        $where['status'] = array('in','1,2');
        $commodity_list = $commodity_model->where($where)->select();
        $this->assign("commodity_list",$commodity_list);
        Log_add(59,'访问行政区划信息列表页面');
        $this->display();
    }
    public function add_commodity(){
        $parent_id = M("commodity")->where(array('parent_id'=>0,'status'=>1))->order('id asc')->select();
...
//                $data['sort'] =$_POST['sort'];
                $data['time'] =time();
                $res = $commodity->where(array('id'=>$_POST['id']))->data($data)->save();
                break;
...
//                $data['path'] =$_POST['path'];
                ...
                $data['status'] =1;
                $commodity_max_code = $commodity->max('id');
                $time = substr(time(),2);
//                $data['code'] = $commodity_max_code.$time;
//                if($_POST['icon'] == 1){
//                    $res = $commodity->where(array('id'=>$_POST['parent_id']))->data(array('icon'=>$_POST['icon']))->save();
//                }else{
//                    $data['icon'] =$_POST['icon'];
//                }
                $res = $commodity->data($data)->add();
